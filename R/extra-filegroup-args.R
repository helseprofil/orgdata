...
is_delete_bydel_before_2003 <- function(dt = NULL, extra = NULL){
  is_debug(deep = TRUE)
  delBydel <- any(grepl("DeleteOldBydel", extra))
...
# Categorise age with S3 object find_age_category()
is_age_category <- function(dt = NULL, extra = NULL){
  is_debug(deep = TRUE)
  ageCat <- any(grepl("AgeCat", extra))

  if (ageCat){
    gp <- is_input_age_class(extra)
...
is_check_age_input <- function(input){
  input <- trimws(input)
  input <- tryCatch(as.numeric(input),
                    warning = function(w){
                      is_stop("Interval for AgeCat in EXTRA is not numeric:", input)
                    })
