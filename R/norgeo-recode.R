...
is_grunnkrets_0000 <- function(dt, control = FALSE){
  ...
  if (nr00 > 0){
    ...
    is_log_write(value = notCodes, x = "code00")
    ...
    is_verbose(x = '`read_log("code00")`', msg = "To see these codes, run command:")
  }
...
is_grunnkrets <- function(dt, control = FALSE){
  ...
  is_log_write(value = notCodes, x = "codeShort")
  ...
  is_verbose(x = '`read_log("codeShort")`', msg = "To see these codes, run command:")
...
is_problem_message <- function(mode, codes, control = FALSE){
  ...
  if (mode == "recode"){
    is_log_write(value = codes, x = "code99")
    ...
    is_verbose(x = '`read_log("code99")`', msg = "To see the codes, run command:")
  }

  if (mode == "delete"){
    is_log_write(value = codes, x = "codeDelete")
    is_verbose(x = length(codes), msg = "Number of geo codes fail to recode and are excluded:", type = "warn2")
    is_verbose(x = scode, msg = "The codes:")
    is_verbose(x = '`read_log("codeDelete")`', msg = "To see these codes, run command:")
  }
