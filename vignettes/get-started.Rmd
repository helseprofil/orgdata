...
using the main function. Otherwise you can install the package using the code below.
...
When cloning the user startup, run the codes in *setup.R* file once before
using *seepafil.R*.
...
```r
df <- read_raw("BEFOLKNING", koblid = 4)
df <- read_raw("BEFOLKNING", c(4,10,15))
```
...
```r
df <- read_raw("BEFOLKNING", aggregate = FALSE)
