...
shorter process time when you run `make_file()` again because it will not re-run
all the cleaning and recording processes as they are allready saved in a
database. The database can be found in *../PRODUKSJON/STYRING/raw-khelse/org_database*
...
specify argument `aggregate` as `FALSE`. You could also activate it globally via
`options`.
...
`aggregate = FALSE`, or you reset the global options with:
