...
OBS! HUSK Å ALLTID TILORDNE ET OBJEKT for `make_file()` og `read_file()`! Ellers blir
ikke resultatene tilgjengelig til lagring eller visning dvs. bruk symbol `<-`.
...
`les_fil()` kan også brukes til å lese data ved å skrive hele stien til filen og kan brukes til å lese filer i flere formater bl.a Excel, Stata, .CSV etc.
...
dt <- lag_fil("BEFOLKNING", raw = TRUE) #les fra original filen hvis allerede er KONTROLLERT
```

## Lagre resultatene eller objekter

Objekt eller resultat fra `lag_file()` ovenfor kan lages til en `.CSV` fil ved å
kjøre:
